...
//
// pq_code layout: num_chunk 4-bit indices packed into
//   num_chunk/2 bytes.  byte[i] = (code[2*i+1] << 4) | code[2*i].
//   num_chunk MUST be even.
//
// lut layout: [num_chunk * 16] floats (kNumCentroids=16 for int4).
...
//
// dist_table layout: [num_chunk * 16 * 16] floats.
