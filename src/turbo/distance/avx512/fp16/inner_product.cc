...
//
// Kernel ported from ailego/math/inner_product_matrix_fp16_avx512.cc:
...
void inner_product_fp16_distance_avx512(const void *a, const void *b,
  ...
  if ((reinterpret_cast<uintptr_t>(lhs) & 0x3f) == 0 &&
    ...
    if (last >= last_aligned + 16) {
      zmm_sum_0 = _mm512_fmadd_ps(_mm512_cvtph_ps(_mm256_load_si256(
    ...
    if (last >= last_aligned + 16) {
      zmm_sum_0 = _mm512_fmadd_ps(_mm512_cvtph_ps(_mm256_loadu_si256(
                                      reinterpret_cast<const __m256i *>(lhs))),
  ...
  if (lhs != last) {
    ...
    zmm_sum_0 = _mm512_fmadd_ps(_mm512_cvtph_ps(_mm256_load_si256(
                                    reinterpret_cast<const __m256i *>(buf_m))),
                                _mm512_cvtph_ps(_mm256_load_si256(
                                    reinterpret_cast<const __m256i *>(buf_q))),
                                zmm_sum_0);
  }
