...
inline __m512i unpack_nibbles_512(const uint8_t *packed) {
  ...
  __m512i hi_mask = _mm512_set_epi8(
      8, 8, 7, 7, 6, 6, 5, 5, 4, 4, 3, 3, 2, 2, 1, 1, 8, 8, 7, 7, 6, 6, 5, 5, 4,
      4, 3, 3, 2, 2, 1, 1, 8, 8, 7, 7, 6, 6, 5, 5, 4, 4, 3, 3, 2, 2, 1, 1, 8, 8,
...
void pq_adc_int4_distance_avx512(const void *pq_code_v, const void *lut_v,
  ...
  const __m512i base_offsets = _mm512_setr_epi32(
      ...
      8 * kNumCentroids, 9 * kNumCentroids, 10 * kNumCentroids,
      11 * kNumCentroids, 12 * kNumCentroids, 13 * kNumCentroids,
      14 * kNumCentroids, 15 * kNumCentroids);
...
void pq_sdc_int4_distance_avx512(const void *a_v, const void *b_v,
  ...
  const __m512i base_offsets = _mm512_setr_epi32(
      ...
      5 * kTablePerSub, 6 * kTablePerSub, 7 * kTablePerSub, 8 * kTablePerSub,
      9 * kTablePerSub, 10 * kTablePerSub, 11 * kTablePerSub, 12 * kTablePerSub,
...
void pq_adc_int4_batch_distance_avx512(const void **candidates_v,
  ...
  for (; i + 4 <= num; i += 4) {
    ...
    for (; m + kChunkSize <= num_subquantizers; m += kChunkSize) {
      ...
      acc0 = _mm512_add_ps(
          acc0, _mm512_i32gather_ps(_mm512_add_epi32(nib0, base_offsets),
                                    lut_base, 4));
      acc1 = _mm512_add_ps(
          acc1, _mm512_i32gather_ps(_mm512_add_epi32(nib1, base_offsets),
                                    lut_base, 4));
      acc2 = _mm512_add_ps(
          acc2, _mm512_i32gather_ps(_mm512_add_epi32(nib2, base_offsets),
                                    lut_base, 4));
      acc3 = _mm512_add_ps(
          acc3, _mm512_i32gather_ps(_mm512_add_epi32(nib3, base_offsets),
                                    lut_base, 4));
    }
