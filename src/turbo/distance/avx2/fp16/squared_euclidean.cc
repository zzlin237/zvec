...
//
// Kernel ported from ailego/math/euclidean_distance_matrix_fp16_avx.cc:
...
void squared_euclidean_fp16_distance_avx2(const void *a, const void *b,
  ...
  if ((reinterpret_cast<uintptr_t>(lhs) & 0xf) == 0 &&
    ...
    if (last >= last_aligned + 8) {
      __m256 ymm_d = _mm256_sub_ps(
          _mm256_cvtph_ps(
              _mm_load_si128(reinterpret_cast<const __m128i *>(lhs))),
          _mm256_cvtph_ps(
              _mm_load_si128(reinterpret_cast<const __m128i *>(rhs))));
  ...
  if (lhs != last) {
    ...
    __m256 ymm_d = _mm256_sub_ps(
        _mm256_cvtph_ps(
            _mm_load_si128(reinterpret_cast<const __m128i *>(buf_m))),
        _mm256_cvtph_ps(
            _mm_load_si128(reinterpret_cast<const __m128i *>(buf_q))));
    ymm_sum_0 = _mm256_fmadd_ps(ymm_d, ymm_d, ymm_sum_0);
  }
