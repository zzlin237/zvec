...
//
// Kernel ported from ailego/math/euclidean_distance_matrix_fp16_neon.cc:
...
// buffer (zero lanes contribute nothing to the squared difference).
...
void squared_euclidean_fp16_distance_neon(const void *a, const void *b,
  ...
  for (; lhs != last_aligned; lhs += 8, rhs += 8) {
    ...
    float32x4_t v_d_0 = vsubq_f32(vcvt_f32_f16(vget_low_f16(v_m)),
                                  vcvt_f32_f16(vget_low_f16(v_q)));
    float32x4_t v_d_1 = vsubq_f32(vcvt_f32_f16(vget_high_f16(v_m)),
...
void squared_euclidean_fp16_batch_distance_neon(const void *const *vectors,
                                                const void *query, size_t n,
                                                size_t dim, float *distances) {
#if defined(__ARM_NEON) && defined(__aarch64__)
  for (size_t i = 0; i < n; ++i) {
    squared_euclidean_fp16_distance_neon(vectors[i], query, dim, &distances[i]);
  }
#else
  (void)vectors;
  (void)query;
