...
//
// Kernel ported from ailego/math/euclidean_distance_matrix_fp32_neon.cc:
// 2x-unrolled dual accumulators and a scalar tail, adapted to the turbo
// DistanceFunc signature.
...
void squared_euclidean_fp32_distance_neon(const void *a, const void *b,
...
void squared_euclidean_fp32_batch_distance_neon(const void *const *vectors,
                                                const void *query, size_t n,
                                                size_t dim, float *distances) {
#if defined(__ARM_NEON) && defined(__aarch64__)
  for (size_t i = 0; i < n; ++i) {
    squared_euclidean_fp32_distance_neon(vectors[i], query, dim, &distances[i]);
  }
#else
  (void)vectors;
  (void)query;
