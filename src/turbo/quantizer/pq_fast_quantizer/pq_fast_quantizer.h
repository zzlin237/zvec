...
//!
//! FastScan is a pure batch-scan quantizer (IVF inverted lists, linear
//! scan).  It provides no SDC and no DistanceImpl handles, so it must not
//! be used for HNSW graph traversal.  Supported metrics: SquaredEuclidean
//! and InnerProduct (IVF handles Cosine by normalize + L2 residual).
class PqFastQuantizer : public Quantizer {
 ...
 private:
  ...
  //! Scalar ADC over a plain nibble-packed code and a quantized query
  //! (packed u8 LUT + delta/bias tail).
  float adc_u8(const uint8_t *code, const uint8_t *qquery) const;
