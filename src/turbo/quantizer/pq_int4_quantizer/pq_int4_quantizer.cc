...
int PqInt4Quantizer::compute_code_norm_table(float *out) const {
...
int PqInt4Quantizer::compute_subspace_ip_table(const void *vec,
                                               float *out) const {
  if (input_data_type_ != DataType::kFp32 || centroids_.empty()) {
    return kErrNotImplemented;
  }
...
int PqInt4Quantizer::preprocess_query(const void *input, float *out) const {
