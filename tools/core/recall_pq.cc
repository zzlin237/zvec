...
// -------------------------------------------------------
// RecallPQ: Test recall for HNSW+PQ with optional refiner
// -------------------------------------------------------
// This tool evaluates recall for a PQ-quantized HNSW index and
// optionally tests the refiner path that re-ranks PQ coarse results
// using a flat FP32 reference index.
//
// Usage: recall_pq CONFIG.yaml [plugin files...]
//
// The YAML config must contain:
//   IndexCommon:
//     IndexConfig, IndexPath, TopK, QueryFile, QueryType, ...
//   QueryConfig:
//     QueryParam: '{"index_type":"kHNSW","ef_search":180}'
...
static void compute_recall_by_id(
  ...
  // Build set of retrieved keys
  set<uint64_t> retrieved_keys;
  for (size_t i = 0; i < knn_res.size() && i < topk; ++i) {
    retrieved_keys.insert(knn_res[i].key());
  }

  // Count matches at each topk level
  for (size_t k = 1; k <= result_size; ++k) {
    size_t match = 0;
    // Check if gt[k-1] key is in the first k retrieved results
...
static void usage(void) {
  cout << "Usage: recall_pq CONFIG.yaml [plugin file path]" << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  // Load PQ index (main index under test)
  string index_dir = config_common["IndexPath"].as<string>();
  core_interface::Index::Pointer pq_index;
  core_interface::BaseIndexQueryParam::Pointer query_param;
  if (parse_and_load_index_param(config_node, index_dir, pq_index,
                                 query_param) != 0) {
    LOG_ERROR("Failed to load PQ index");
    return -1;
  }

  // Load flat reference index for ground truth generation
  // We look for RefinerConfig.ReferenceIndex to get the flat index config
  ...
  if (auto refiner_config = query_config["RefinerConfig"]; refiner_config) {
    ...
    // No refiner config - look for a separate FlatIndexPath for GT generation
    if (config_common["FlatIndexPath"]) {
      string flat_path = config_common["FlatIndexPath"].as<string>();
      // Build a minimal flat index param
