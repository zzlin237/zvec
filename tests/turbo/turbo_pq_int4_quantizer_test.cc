...
// Build a PqInt4Quantizer with an optional OPQ rotation.
...
TEST(PqInt4Opq, ReconstructionErrorLowerThanBaseline) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  const size_t COUNT = 2000;
  ...
  std::cout << "PqInt4 reconstruction mse: base=" << base_mse
...
TEST(PqInt4Opq, RecallNotWorseThanBaseline) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  const size_t COUNT = 1000;
  ...
  std::cout << "PqInt4 recall@" << TOPK << ": base=" << base_recall
            << " opq=" << opq_recall << std::endl;

  EXPECT_GE(opq_recall, base_recall);
}

TEST(PqInt4Opq, CosineDequantizeRestoresNorm) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  const size_t COUNT = 1000;
...
TEST(PqInt4Opq, SerializeDeserializeKeepsRotation) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  ...
  auto restored = IndexFactory::CreateQuantizer("PqInt4Quantizer");
  ASSERT_TRUE(restored);
  ASSERT_EQ(0, restored->deserialize(opq_blob));

  const size_t code_bytes = opq->quantized_datapoint_vector_length();
  std::vector<uint8_t> code1(code_bytes);
...
TEST(PqInt4Opq, InitRejectsUnsupportedConfigurations) {
  auto q = IndexFactory::CreateQuantizer("PqInt4Quantizer");
  ASSERT_TRUE(q);
  IndexMeta meta;
  meta.set_meta(IndexMeta::DataType::DT_FP32, 16);
