...
TEST(PqFastQuantizer, InitInvalidParams) {
  ...
  // Cosine is not supported (IVF normalizes and searches with L2).
  EXPECT_EQ(nullptr, make_pqfs_quantizer(16, 4, "Cosine"));
}
...
TEST(PqFastScanKernel, DispatchTableIsFamilyExclusive) {
  ...
  // kPQFast fills only fast_scan; FastScan has no SDC / single-code path.
  auto fast = get_pq_kernels(DataType::kInt4, QuantizeType::kPQFast);
  EXPECT_TRUE(fast.fast_scan);
  EXPECT_FALSE(fast.adc_distance);
