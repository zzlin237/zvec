...
// Build a PqFastQuantizer with an optional OPQ rotation.
...
TEST(PqFastOpq, ReconstructionErrorLowerThanBaseline) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  ...
  std::cout << "PqFast reconstruction mse: base=" << base_mse
...
TEST(PqFastOpq, RecallNotWorseThanBaseline) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  const size_t COUNT = 1000;
  ...
  auto measure_recall = [&](const std::shared_ptr<zvec::turbo::Quantizer> &q) {
    ...
    // FastScan queries are affine-quantized u8 LUTs, not float tables.
    std::vector<uint8_t> qquery(q->quantized_query_vector_length());
    ...
    for (size_t qi = 0; qi < QUERIES; ++qi) {
      ...
      q->quantize_query(query.data(), qquery.data());
      std::vector<std::pair<float, size_t>> adc;
      adc.reserve(raw.size());
      for (size_t i = 0; i < raw.size(); ++i) {
        adc.emplace_back(
            q->calc_distance_dp_query(codes[i].data(), qquery.data()), i);
      }
  ...
  std::cout << "PqFast recall@" << TOPK << ": base=" << base_recall
            << " opq=" << opq_recall << std::endl;

  EXPECT_GE(opq_recall, base_recall);
}

TEST(PqFastOpq, CosineDequantizeRestoresNorm) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  const size_t COUNT = 1000;
  ...
  for (auto iter = holder->create_iterator(); iter->is_valid() && checked < 20;
    ...
    // so the reconstruction keeps the original magnitude.
    EXPECT_NEAR(rec_norm, raw_norm, raw_norm * 0.45f);
    EXPECT_GT(dot / (raw_norm * rec_norm + 1e-12f), 0.4f);
  }
  EXPECT_EQ(20u, checked);
}

TEST(PqFastOpq, SerializeDeserializeKeepsRotation) {
  const size_t DIM = 32;
  const size_t NSQ = 8;
  ...
  auto restored = IndexFactory::CreateQuantizer("PqFastQuantizer");
  ASSERT_TRUE(restored);
  ASSERT_EQ(0, restored->deserialize(opq_blob));

  const size_t packed_bytes = (NSQ + 1) / 2;
  ...
  std::vector<uint8_t> q1(opq->quantized_query_vector_length());
  std::vector<uint8_t> q2(restored->quantized_query_vector_length());
  ...
  for (auto iter = holder->create_iterator(); iter->is_valid() && checked < 20;
    ...
    for (size_t b = 0; b < packed_bytes; ++b) {
      ASSERT_EQ(code1[b], code2[b]) << "checked=" << checked << " byte=" << b;
    }
    opq->quantize_query(iter->data(), q1.data());
    restored->quantize_query(iter->data(), q2.data());
    EXPECT_NEAR(opq->calc_distance_dp_query(code1.data(), q1.data()),
                restored->calc_distance_dp_query(code2.data(), q2.data()),
                1e-4f);
  }
  EXPECT_EQ(20u, checked);
}

TEST(PqFastOpq, InitRejectsUnsupportedConfigurations) {
  auto q = IndexFactory::CreateQuantizer("PqFastQuantizer");
  ASSERT_TRUE(q);
  IndexMeta meta;
  meta.set_meta(IndexMeta::DataType::DT_FP32, 16);
